...
class TestS3CopyObjects(S3Api):
    ...
    async def execute_copy_object_workload(self):
        ...
        bucket_name1 = f'copy-obj-bkt1-{self.test_id}-{session_id}-{perf_counter_ns()}'.lower()
        bucket_name2 = f'copy-obj-bkt2-{self.test_id}-{session_id}-{perf_counter_ns()}'.lower()
        object_name1 = f'copy-obt1-{self.test_id}-{session_id}-{perf_counter_ns()}'.lower()
        object_name2 = f'copy-obt2-{self.test_id}-{session_id}-{perf_counter_ns()}'.lower()
        ...
        while True:
            try:
                ...
                self.log.info("Object1 '%s', object size %s", object_name1,
                ...
                await self.copy_object(bucket_name1, object_name1, bucket_name2, object_name2)
                ...
                if self.range_read:
                    ...
                    await self.data_integrity(bucket_name1, bucket_name2, object_name1,
                                              object_name2,
                                              byte_range=f'bytes={offset}-{range_read + offset}')
                else:
                    self.log.info("Download source and destination object and compare checksum.")
                    await self.data_integrity(bucket_name1, bucket_name2, object_name1, object_name2)
    ...
    async def data_integrity(self, bucket_name1, bucket_name2, object_name1, object_name2,
