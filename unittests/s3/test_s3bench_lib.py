...
class S3benchTestCase(unittest.TestCase):
    ...
    def test_check_log_file_error_report(self, mock_install_s3bench):
        ...
        with mock.patch(
            "builtins.open", mock.mock_open(read_data=json.dumps(s3bench_run_report))
        ) as _:
            self.assertEqual(
                s3bench.check_log_file_error("", ""),
                (
                    False,
                    {
                        "Write Errors": 0,
    ...
    def test_check_log_file_error_cli(self, mock_install_s3bench):
        ...
        with mock.patch("builtins.open", mock.mock_open()) as mock_open:
            mock_open.side_effect = [
                StringIO(json.dumps(s3bench_run_report) + "corrupt"),
                StringIO(
                    r"^MWrite | 0/400 (0.00%) | time 0s eta 0s | errors 0"
                    r"                               ^MWrite | 1/400 (0.2"
    ...
    def test_check_log_file_error(self, mock_install_s3bench):
        ...
        s3bench_run_report = {
            "Parameters": {"copies": 0},
            "Tests": [
                {"Errors Count": 0, "Operation": "Write"},
