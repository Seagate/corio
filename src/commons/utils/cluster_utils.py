...
class BaseClusterServices(RemoteHost):
    ...
    def send_k8s_cmd(
        self, operation: str, pod: str, namespace: str, command_suffix: str, **kwargs
    ) -> bytes:
    ...
    def get_all_pods_containers(self, pod_prefix, pod_list=None):
        ...
        if not pod_list:
            ...
            output = self.execute_command(
                cmd.CMD_POD_STATUS + " -o=custom-columns=NAME:.metadata.name",
                read_lines=True,
            )
            for lines in output:
...
class ClusterServices(BaseClusterServices):
    ...
    def get_hctl_status(self):
        ...
        if status:
            ...
            status, output = self.exec_k8s_command(
                cmd.CMD_K8S_CLUSTER_HEALTH.format(
            ...
            for node in response["nodes"]:
                ...
                for service in services:
                    if service["status"] != "started":
                        LOGGER.error(
                            "%s service not started on pod %s",
                            service["name"],
                            pod_name,
                        )
    ...
    def collect_support_bundles(self, dir_path: str) -> tuple:
        ...
        status, response = self.exec_k8s_command(
            cmd.CMD_GENERATE_CLSTR_LOGS.format(script_path, const.K8S_SB_SCRIPT),
            read_lines=True,
        )
        if not status:
    ...
    def create_pod_replicas(self, num_replica, deploy=None, pod_name=None):
        ...
        status, output = self.execute_command(command=k8s_cmd, read_lines=True)
        LOGGER.info(
            "Deployment exists is: %s after POD: %s Replica set to %s",
            status,
            deploy,
            num_replica,
        )
    ...
    def get_deploy_replicaset(self, pod_name):
        ...
        try:
            ...
            LOGGER.error(
                ...
                ClusterServices.get_deploy_replicaset.__name__,
                error,
            )
            return False, error
    ...
    def get_num_replicas(self, replicaset):
        ...
        try:
            ...
            LOGGER.error(
                "*ERROR* An exception occurred in %s: %s",
                ClusterServices.get_num_replicas.__name__,
                error,
            )
            return False, error
    ...
    def recover_deployment_k8s(self, backup_path, deployment_name):
        ...
        try:
            ...
            LOGGER.error(
                "*ERROR* An exception occurred in %s: %s",
                ClusterServices.recover_deployment_k8s.__name__,
                error,
            )
            return False, error
    ...
    def backup_deployment(self, deployment_name):
        ...
        try:
            ...
            LOGGER.error(
                "*ERROR* An exception occurred in %s: %s",
                ClusterServices.backup_deployment.__name__,
                error,
            )
            return False, error
    ...
    def degrade_nodes(self, pod_prefix=const.DATA_POD_NAME_PREFIX) -> tuple:
        ...
        status, resp_ns = self.execute_command(command=k8s_cmd, read_lines=True)
        if status:
            LOGGER.info(resp_ns)
        ...
        for _ in range(int(os.environ["DEGRADED_PODS_CNT"])):
            ...
            LOGGER.info(
                "Shutdown/deleted pod %s for host %s with replicas=0",
                pod_name,
                hostname,
            )
    ...
    def get_cluster_config(self, pod_list=None) -> dict:
        ...
        pod_name = random.sample(pod_list, 1)[0]
        conf_cp = cmd.K8S_CP_TO_LOCAL_CMD.format(
            pod_name,
            const.CLUSTER_CONF_PATH,
            const.LOCAL_CONF_PATH,
            const.HAX_CONTAINER_NAME,
        )
    ...
    def get_user_quota_in_bytes(self):
        ...
        LOGGER.info(
            "Total Capacity: %s Available Capacity: %s Used Capacity: %s",
            total_capacity,
            avail_capacity,
            used_capacity,
        )
